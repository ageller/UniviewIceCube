...
class EventViewer2D:
    ...
    def DrawGeometryXProj(self):
        # x projection
        #AMG edit for python3
        if (sys.version_info < (3, 0)):
            x = [pos[0] for om,pos in self.geo.iteritems() if om[0] ]
            y = [pos[2] for om,pos in self.geo.iteritems() if om[0] ]
        else:
            x = [pos[0] for om,pos in self.geo.items() if om[0] ]
            y = [pos[2] for om,pos in self.geo.items() if om[0] ]
        p.ylabel("z(m)")
        p.xlabel("x(m)")
    ...
    def DrawGeometryYProj(self):
        ...
        if (sys.version_info < (3, 0)):
            ...
            x = [pos[1] for om,pos in self.geo.items() if om[0] ]
            y = [pos[2] for om,pos in self.geo.items() if om[0] ]
        #p.ylabel("z(m)")
        p.xlabel("y(m)")
